...
#include "GUI/Components/lib/qtmaterialstatetransition.h"
...
QtMaterialButtonStateMachine::QtMaterialButtonStateMachine(QtMaterialButton *parent)
      ...
      m_wasChecked(false)
{
    ...
    transition = new QtMaterialStateTransition(TextButtonCheckedTransition);
    ...
    transition = new QtMaterialStateTransition(TextButtonUncheckedTransition);
    ...
    transition = new QtMaterialStateTransition(TextButtonPressedTransition);
...
QtMaterialButtonStateMachine::~QtMaterialButtonStateMachine()
{
}

void QtMaterialButtonStateMachine::setOverlayOpacity(qreal opacity)
{
    m_overlayOpacity = opacity;
    m_button->update();
}

void QtMaterialButtonStateMachine::setCheckedOverlayProgress(qreal progress)
{
    m_checkedOverlayProgress = progress;
    m_button->update();
}

void QtMaterialButtonStateMachine::setHaloOpacity(qreal opacity)
{
    m_haloOpacity = opacity;
    m_button->update();
}

void QtMaterialButtonStateMachine::setHaloSize(qreal size)
{
    m_haloSize = size;
    m_button->update();
}

void QtMaterialButtonStateMachine::setHaloScaleFactor(qreal factor)
{
    m_haloScaleFactor = factor;
    m_button->update();
}

void QtMaterialButtonStateMachine::startAnimations()
{
    start();
}

void QtMaterialButtonStateMachine::setupProperties()
{
    QColor overlayColor;
...
void QtMaterialButtonStateMachine::updateCheckedStatus()
{
    const bool checked = m_button->isChecked();
    if (m_wasChecked != checked) {
        m_wasChecked = checked;
        if (checked) {
            postEvent(new QtMaterialStateTransitionEvent(TextButtonCheckedTransition));
        } else {
            postEvent(new QtMaterialStateTransitionEvent(TextButtonUncheckedTransition));
        }
    }
}

bool QtMaterialButtonStateMachine::eventFilter(QObject *watched,
                                                   QEvent  *event)
{
    if (QEvent::FocusIn == event->type()) {
        QFocusEvent *focusEvent = static_cast<QFocusEvent *>(event);
        if (focusEvent && Qt::MouseFocusReason == focusEvent->reason()) {
            postEvent(new QtMaterialStateTransitionEvent(TextButtonPressedTransition));
            return true;
        }
    }
...
void QtMaterialButtonStateMachine::addTransition(QObject *object,
...
void QtMaterialButtonStateMachine::addTransition(QAbstractTransition *transition,
                                                     QState *fromState,
                                                     QState *toState)
{
